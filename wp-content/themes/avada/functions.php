		...
		if ( is_category() ) {
			...
			if ( get_post_type() != 'post' ) {
				...
				// foreach ($categories as $category) {
					...
					// if($parentId > 0) {
						// $countBranch++;
						// $parentObj = get_category($parentId);
						// if($parentObj->category_parent > 0) {
							
							// echo $cat_ancestor_name = get_cat_name($parentObj->category_parent);
							// $branch .=
							// '<span class="breadcrumb-cat-last" typeof="v:Breadcrumb">
								// <a rel="v:url" property="v:title" href="' . get_category_link($parentObj->category_parent) . '" title="View all posts in ' . $cat_ancestor_name . '">' . $cat_ancestor_name . '</a>
							// </span>';
							// //$branch .= walk_branch($parentObj,$countBranch);
						// }
					// }
				...
				function walk_branch($category,$countBranch = 0) {
					...
					if($parentId > 0) {
						...
						if($parentObj->category_parent > 0) {
							$countBranch++;
							$cat_ancestor_name = get_cat_name($parentObj->category_parent);
							$branch .= walk_branch($parentObj,$countBranch);
						}
						//if($countBranch > 0) {
							$branch .=
							'<span class="breadcrumb-cat-arrow" typeof="v:Breadcrumb">
        		...
        		// foreach ($categories as $category) {
					...
					// if (!in_array($ancestorId, $ancestors)) {
						// $ancestors[] = $ancestorId;
						// if(count($ancestors) > 0) {
							// $cat_parent_name = get_cat_name($category->parent);
							// $result .=
							// '<span typeof="v:Breadcrumb" class="breadcrumb-cat-last">
								// <a rel="v:url" property="v:title" href="' . get_category_link($category->parent) . '" title="View all posts in ' . $cat_parent_name . '">' . $cat_parent_name . '</a>
							// </span>';
						// }
						// walk_tree($category);
...
function register_jquery() {
    ...
    wp_register_script( 'jquery', ( '//cdnjs.cloudflare.com/ajax/libs/jquery/1.11.1/jquery.min.js' ), false, null, false );
...
function register_jquery_migrate() {
    ...
    wp_register_script( 'jquery-migrate', ( '//cdnjs.cloudflare.com/ajax/libs/jquery-migrate/1.2.1/jquery-migrate.min.js' ), false, null, false );
    wp_enqueue_script( 'jquery-migrate' );
}

add_action( 'wp_enqueue_scripts', 'register_jquery_ui_js' );
function register_jquery_ui_js() {
	global $wp_scripts;
	
	$queryui = $wp_scripts->query('jquery-ui-core');
	
    wp_deregister_script( 'jquery-ui' );
    wp_register_script( 'jquery-ui', ( '//cdnjs.cloudflare.com/ajax/libs/jqueryui/'.$queryui->ver.'/jquery-ui.min.js' ), false, null, true );
    wp_enqueue_script( 'jquery-ui' );
}

add_action( 'wp_enqueue_scripts', 'register_bootstrap_js' );
function register_bootstrap_js() {
    wp_register_script( 'bootstrap-js', ( '//maxcdn.bootstrapcdn.com/bootstrap/3.2.0/js/bootstrap.min.js' ), false, null, true );
    wp_enqueue_script( 'bootstrap-js' );
}

//add_action( 'wp_enqueue_scripts', 'register_bootstrap_css' );
function register_bootstrap_css() {
    wp_register_style( 'bootstrap-css', ( '//maxcdn.bootstrapcdn.com/bootstrap/3.2.0/css/bootstrap.min.css' ), false, null, true );
    wp_enqueue_style( 'bootstrap-css' );
}

add_action( 'wp_enqueue_scripts', 'register_jquery_ui_css' );
function register_jquery_ui_css() {
	global $wp_scripts;
    ...
    //wp_register_style( 'jquery-ui', ( '//ajax.googleapis.com/ajax/libs/jqueryui/'.$queryui->ver.'/themes/smoothness/jquery-ui.css' ), false, null, false );
...
function de_style() {
    ...
    wp_deregister_style( 'usernoise-adminbar' );
	
	// Dequeue jQuery
	wp_dequeue_style( 'usernoise-button' );
    wp_deregister_style( 'usernoise-button' );
	
	// Dequeue usernoise
	wp_dequeue_style( 'usernoise-form' );
    wp_deregister_style( 'usernoise-form' );
	...
	// Dequeue jnewsticker
    wp_dequeue_style( 'ajaxy-sf-common' );
    wp_deregister_style( 'ajaxy-sf-common' );
	
	// Dequeue kkstar ratings
    wp_dequeue_style( 'ajaxy-sf-selective' );
    wp_deregister_style( 'ajaxy-sf-selective' );
	
	// Dequeue usernoise button
    wp_dequeue_style( 'ajaxy-sf-theme' );
    wp_deregister_style( 'ajaxy-sf-theme' );
	...
	// Dequeue usernoise button
    wp_dequeue_style( 'slider-pro-slider-base' );
    wp_deregister_style( 'slider-pro-slider-base' );
	
	// Dequeue usernoise button
    wp_dequeue_style( 'slider-pro-skin-glossy-curved-square-gray' );
    wp_deregister_style( 'slider-pro-skin-glossy-curved-square-gray' );
	
	// Dequeue usernoise button
    wp_dequeue_style( 'userpro_google_font' );
	...
	// Dequeue usernoise button
    wp_dequeue_style( 'userpro_lightview' );
    wp_deregister_style( 'userpro_lightview' );
	
	// Dequeue usernoise button
    wp_dequeue_style( 'userpro_jquery_ui_style' );
	...
	// Dequeue usernoise button
    wp_dequeue_style( 'avada-animations' );
    wp_deregister_style( 'avada-animations' );
	
	// Dequeue usernoise button
    wp_dequeue_style( 'avada-media' );
    wp_deregister_style( 'avada-media' );
	
	// Dequeue usernoise button
    wp_dequeue_style( 'wpb' );
    wp_deregister_style( 'wpb' );
	
	// Dequeue usernoise button
    wp_dequeue_style( 'wpb_iconfont' );
    wp_deregister_style( 'wpb_iconfont' );
	
	// Dequeue usernoise button
    wp_dequeue_style( 'wpb_list' );
    wp_deregister_style( 'wpb_list' );
	
	// Dequeue usernoise button
    wp_dequeue_style( 'wpb_chosen' );
    wp_deregister_style( 'wpb_chosen' );
	
	// Dequeue usernoise button
    wp_dequeue_style( 'wpmudev-chat-wpadminbar-style' );
    wp_deregister_style( 'wpmudev-chat-wpadminbar-style' );
	
	// Dequeue usernoise button
    wp_dequeue_style( 'wpmudev-chat-style' );
    wp_deregister_style( 'wpmudev-chat-style' );
...
function de_script() {
    ...
    wp_deregister_script( 'usernoise-button' );
	
	// Dequeue jnewsticker
    wp_dequeue_script( 'jnewsticker' );
    wp_deregister_script( 'jnewsticker' );
	...
	// Dequeue usernoise button
    wp_dequeue_script( 'userpro_swf' );
    wp_deregister_script( 'userpro_swf' );
	
	// Dequeue usernoise button
    wp_dequeue_script( 'userpro_spinners' );
    wp_deregister_script( 'userpro_spinners' );
	
	// Dequeue usernoise button
    wp_dequeue_script( 'userpro_lightview' );
    wp_deregister_script( 'userpro_lightview' );
	
	// Dequeue usernoise button
    wp_dequeue_script( 'userpro_min' );
    ...
    wp_deregister_script( 'wpuf-upload' );
	
	// Dequeue usernoise button
    wp_dequeue_script( 'fixjs' );
    wp_deregister_script( 'fixjs' );
	...
	// Dequeue usernoise button
    wp_dequeue_script( 'userpro_ed' );
    wp_deregister_script( 'userpro_ed' );
	
	// Dequeue usernoise button
    wp_dequeue_script( 'wpb_chosen' );
    wp_deregister_script( 'wpb_chosen' );
	
	// Dequeue usernoise button
    wp_dequeue_script( 'wpb' );
    wp_deregister_script( 'wpb' );
    ...
    wp_deregister_script( 'jm_like_post' );
	
	// Dequeue usernoise button
    wp_dequeue_script( 'ajaxy-sf-search' );
    wp_deregister_script( 'ajaxy-sf-search' );
	
	// Dequeue usernoise button
    wp_dequeue_script( 'ajaxy-sf-selective' );
    wp_deregister_script( 'ajaxy-sf-selective' );
    ...
    wp_deregister_script( 'bootstrap-scripts' );
	...
	// Dequeue usernoise button
    wp_dequeue_script( 'formstone-selecter' );
    wp_deregister_script( 'formstone-selecter' );
...
if(is_front_page()) { 
	...
	//add_action('wp_enqueue_scripts', 'single_js');
}
...
if ( ! function_exists( 'cor_remove_personal_options' ) ) {
  ...
  function cor_remove_personal_options( $subject ) {
    $subject = preg_replace( '#<h3>Chat Settings</h3>.+?</table>.+?</table>#s', '', $subject, 1 );
    return $subject;
  }
...
add_action( 'bbp_user_edit_before', 'cor_profile_subject_start' );
add_action( 'bbp_user_edit_after', 'cor_profile_subject_end' );



// Change full headers to send HTML formatted emails
...
// Redirect log out user to current page
...
function add_loginout_link( $items, $args ) {
	...
	if(!in_array($_SERVER['REMOTE_ADDR'], $whitelist)){
		$first_subdirectory = $parts[1];
	} else {
		$first_subdirectory = $parts[2];
	}
	
    // if (is_user_logged_in() && $args->theme_location == 'main_navigation') {
	if(is_front_page()) {
		$itemsHolder = $items;
		$newItem = '<li class="menu-item current-menu-item fusion-dropdown-menu"><a href="' . get_site_url() . '"><i class="fa fa-home"> </i></a></li>';
		$forum = '<li class="menu-item fusion-dropdown-menu"><a href="' . get_site_url() . '/forums/">Forum</a></li>';
		$items = '';
        $items .= $newItem.$itemsHolder.$forum;
    }
    //elseif (!is_user_logged_in() && $args->theme_location == 'main_navigation') {
    else if($first_subdirectory == 'forums') {
    	$itemsHolder = $items;
		$newItem = '<li class="menu-item fusion-dropdown-menu"><a href="' . get_site_url() . '"><i class="fa fa-home"> </i></a></li>';
		$forum = '<li class="menu-item current-menu-item fusion-dropdown-menu"><a href="' . get_site_url() . '/forums/">Forum</a></li>';
		$items = '';
        $items .= $newItem.$itemsHolder.$forum;	
    } else {

...
class WPMUDEV_Chat {
	var $chat_current_version = '2.0.8.5';
	...
	function __construct() {
		...
		$wpmudev_notices[] = array( 'id'=> 159,'name'=> 'WordPress Chat', 'screens' => array( 'toplevel_page_chat_settings_panel', 'chat_page_chat_settings_panel_site', 'chat_page_chat_settings_panel_widge', 'chat_page_chat_settings_panel_global', 'chat_page_chat_session_logs') );
...
//		}
	...
	function admin_enqueue_scripts() {
		...
		//echo "_show_own_admin[". $this->_show_own_admin ."]<br />";
...
//		if ($this->get_option('box_shadow_show', 'site') == "enabled") {
	...
	function admin_notices() {
		...
		if (!$nag_version) {
			?><div id="wpmudev-chat-nag-error" class="error below-h2"><p><?php echo sprintf( __('You have installed the WordPress Chat plugin. <strong>Please review the <a href="%s">polling interval</a> settings before using</strong>. In most cases the default options will work for your host configuration. However, in some unique cases the polling interval is too frequent and could cause your hosting to shutdown your account. Once you have reviewed and saved the chat settings this message will no longer appear.', $this->translation_domain ), esc_url( $chat_admin_url )) ?></p></div><?php
		} else if (version_compare($this->_chat_plugin_settings['options_version'], $nag_version) > 0) {
			?><div id="wpmudev-chat-nag-error" class="error"><p><?php echo sprintf( __('The WordPress Chat plugin has been upgraded on yor site. <strong>Please review the <a href="%s">polling interval</a> settings before using</strong>. In most cases the default options will work for your host configuration. However in some unique cases the polling interval is too frequent and could cause your hosting to shutdown your account. Once you have reviewed and saved the chat settings this message will no longer appear.', $this->translation_domain ), esc_url( $chat_admin_url )) ?></p></div><?php
		}
	}
	...
	function network_admin_menu() {
		...
		*/
		...
		add_action( 'load-'. $this->_pagehooks['chat_settings_panel_network_dashboard'], 	array(&$this, 'on_load_panels') );
		add_action( 'load-'. $this->_pagehooks['chat_settings_panel_network_global'], 		array(&$this, 'on_load_panels') );
		add_action( 'load-'. $this->_pagehooks['chat_session_logs_network_site'], 			array(&$this, 'on_load_panels') );
	}
	...
	function admin_menu() {
		...
		add_action( 'load-'. $this->_pagehooks['chat_settings_panel_dashboard'], 	array(&$this, 'on_load_panels') );
	...
	function on_load_panels() {
		...
		if ((isset($_GET['page'])) && ($_GET['page'] == "chat_session_logs")) {
			...
			if ((isset($_GET['maction'])) && (isset($_GET['_wpnonce'])) && (wp_verify_nonce( $_GET['_wpnonce'], 'chat-message-item' ))) {
				...
				$_SERVER['REQUEST_URI'] = remove_query_arg( array( '_wp_http_referer', '_wpnonce', 'action', 'action2', 'mid' ), $_SERVER['REQUEST_URI'] );

				if (($bulk_action) && (!empty($bulk_action))) {
					$chat_href = remove_query_arg(array('_wpnonce', 'maction', 'message'), $_GET['_wp_http_referer']);
	...
	function chat_edit_user_profile( $user = '') {
	    ...
	    <table class="form-table">
		    ...
		    <tr class="wpmudev_chat_wp_admin_display" <?php if ($user_meta['chat_wp_admin'] != 'enabled') { echo ' style="display:none;" '; } ?>>
		        ...
		        <td>
					...
					<div id="wpmudev_chat_wp_toolbar_options" <?php if ( $user_meta['chat_wp_toolbar'] == 'disabled' ) { echo ' style="display:none;" '; }  ?> >
			            <p><select name="wpmudev_chat_user_settings[chat_wp_toolbar_show_status]" id="wpmudev_chat_wp_toolbar_show_status">
			            		<option value="enabled"<?php if ( $user_meta['chat_wp_toolbar_show_status'] == 'enabled' ) { echo ' selected="selected" '; } ?>><?php _e('Enabled', $this->translation_domain); ?></option>
			            		<option value="disabled"<?php if ( $user_meta['chat_wp_toolbar_show_status'] == 'disabled' ) { echo ' selected="selected" '; } ?>><?php _e('Disabled', $this->translation_domain); ?></option>
			            	</select> <label for="wpmudev_chat_wp_toolbar_show_status"><?php _e('Show Your Chat Status on WordPress toolbar menu?', $this->translation_domain); ?></label></p>

			            <p><select name="wpmudev_chat_user_settings[chat_wp_toolbar_show_friends]" id="wpmudev_chat_wp_toolbar_show_friends">
			            		<option value="enabled"<?php if ( $user_meta['chat_wp_toolbar_show_friends'] == 'enabled' ) { echo ' selected="selected" '; } ?>><?php _e('Enabled', $this->translation_domain); ?></option>
			            		<option value="disabled"<?php if ( $user_meta['chat_wp_toolbar_show_friends'] == 'disabled' ) { echo ' selected="selected" '; } ?>><?php _e('Disabled', $this->translation_domain); ?></option>
			            	</select> <label for="wpmudev_chat_wp_toolbar_show_friends"><?php _e('Show Your Chat Friends on WordPress toolbar menu?', $this->translation_domain); ?></label></p>
					</div>
					 ...
					 || (array_intersect($this->_chat_options['dashboard']['login_options'], $user->roles))) {
					    ...
					    <tr class="wpmudev_chat_wp_admin_display" <?php if ($user_meta['chat_wp_admin'] != 'enabled') { echo ' style="display:none;" '; } ?>>
					        ...
					        <td>
								...
								<div id="wpmudev_chat_dashboard_widget_options" <?php if ( $user_meta[$item_key] == 'disabled' ) { echo ' style="display:none;" '; } ?>
									<?php
										if (is_network_admin()) {
						            ...
						            <p><label for="wpmudev_chat_dashboard_widget_height"><?php _e('Height of Chat Widget', $this->translation_domain); ?></label><br /><input name="wpmudev_chat_user_settings[<?php echo $item_option_key; ?>]" id="wpmudev_chat_dashboard_widget_height" value="<?php echo $user_meta[$item_option_key]; ?>" /> <?php _e('Default is', $this->translation_domain)?> <?php echo $this->_chat_options['dashboard']['dashboard_widget_height'] ?></p>
								</div>
					        </td>
						</tr>
						<?php
					 ...
					 && ((is_multisite()) && (!is_plugin_active_for_network('friends/friends.php')))) {
					    ...
					    <tr class="wpmudev_chat_wp_admin_display" <?php if ($user_meta['chat_wp_admin'] != 'enabled') { echo ' style="display:none;" '; } ?>>
					        <th><label for="wpmudev_chat_dashboard_friends_widget"><?php
								_e('Show Chat Friends Widget on Dashboard', $this->translation_domain); ?></label></th>
					        <td>
								<?php
									if (is_network_admin()) {
								...
								<div id="wpmudev_chat_dashboard_friends_widget_options" <?php if ( $user_meta[$item_key] == 'disabled' ) { echo ' style="display:none;" '; } ?>
									<?php
										if (is_network_admin()) {
	...
	function chat_delete_user($id, $reassign = null) {
		...
		if ($this->get_option('delete_user_messages', 'global') != 'enabled') return;
...
//		}
		...
		if (($chat_session['session_type'] == 'page') || ($chat_session['session_type'] == 'bp-group')) {
			if ($chat_session['session_type'] == 'page') {
				...
				$chat_session['session_title'] 	= apply_filters_ref_array( 'bp_get_group_name', array( $current_group->name, $current_group ) );
	...
	function chat_logs_container($chat_session) {
		...
		if (($chat_session['log_display'] == "enabled-list-above") || ($chat_session['log_display'] == "enabled-list-below")) {
			...
			if (isset($_GET['chat-show-logs'])) {
				...
				$return_link = '<a href="'. remove_query_arg(array('chat-log-id', 'chat-show-logs')) .'">'. __('Return to Chat', $this->translation_domain) .'</a>';
...
//					if ($this->get_option('box_shadow_show', 'network-site') == "enabled") {
	...
	function chat_box_header_container($chat_session) {
		...
		if ($chat_session['session_type'] != "bp-group") {
			$chat_header_images .= '<img class="wpmudev-chat-min" src="'. plugins_url('/images/down.png', __FILE__)  .'" alt="-" width="16" height="16" style="'. $chat_style_min .'" title="'. __('Minimize Chat', $this->translation_domain) .'" />';
			$chat_header_images .= '<img class="wpmudev-chat-max" src="'. plugins_url('/images/up.png', __FILE__) .'" alt="+" width="16" height="16" style="'. $chat_style_max .'" title="'. __('Maximize Chat', $this->translation_domain) .'" />';
...
//		}
		...
		$content .= '<span class="wpmudev-chat-title-text"><i class="fa fa-comments-o fa-lg"></i> <span class="chat-title">Group Chat</span></span>';
	...
	function chat_session_build_row($row, $chat_session) {
		...
		$row_avatar_name = '';
			...
			if (!empty($row_date_time))
				$row_date_time = $row_date_time . "<br />";

			$row_text .= '<p class="wpmudev-chat-message">'. $row_date_time . '<strong>'.$row_avatar_name.'</strong>'.' : '. convert_smilies($message) . '</p>';
		...
		switch($function) {
			...
			case 'chat_message_send':
				...
				foreach($_POST['chat_messages'] as $chat_id => $chat_messages) {
					...
					if (!isset($this->chat_sessions[$chat_id])) continue;
					if ((!is_array($chat_messages)) || (!count($chat_messages))) continue;

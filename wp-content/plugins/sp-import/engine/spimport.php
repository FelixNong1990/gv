...
function spi_import_users($phase, $source, $def, $map) {
	...
	$dbConn = mysql_connect($source->dbhost, $source->dbuser, $source->dbpass);
	...
	$dbResourceUser = mysql_query($sql, $dbConn);
	while($spiUser = mysql_fetch_array($dbResourceUser, MYSQL_ASSOC)) {
...
function spi_import_forums($phase, $source, $def, $map) {
	...
	if($phase == 1) {
		if($map['tables']->sfgroups == false) {
			...
			$dbResourceGroup = mysql_query($sql, $dbConn);
			while($dbGroup = mysql_fetch_array($dbResourceGroup, MYSQL_ASSOC)) {
	...
	$dbResourceForum = mysql_query($sql, $dbConn);

	$dbForum = mysql_fetch_array($dbResourceForum, MYSQL_ASSOC);
	...
	$dbResourceTopic = mysql_query($sql, $dbConn);
	while($dbTopic = mysql_fetch_array($dbResourceTopic, MYSQL_ASSOC)) {
		...
		if($tslug != false) {
			...
			$dbResourcePost = mysql_query($sql, $dbConn);

			while($dbPost = mysql_fetch_array($dbResourcePost, MYSQL_ASSOC)) {
				# Create the post
...
function spi_import_tags($source, $def) {
	...
	$dbResourceTag = mysql_query($sql, $dbConn);
	while($bbTag = mysql_fetch_array($dbResourceTag, MYSQL_ASSOC)) {
		# Create the tag
	...
	if(array_key_exists($column, $row)) {
		...
		switch($special[0]) {
			...
			case '#query':
				...
				$dbConn = mysql_connect($source->dbhost, $source->dbuser, $source->dbpass, true);
				if(!$dbConn) echo 'NO CONNECTION';
				$dbResourceQuery = mysql_query($sql, $dbConn);
				$dbData = mysql_fetch_array($dbResourceQuery, MYSQL_NUM);
				$data = $dbData[0];
				mysql_close($dbConn);
				break;
		}

...
{
	$conn = @mysql_connect($db->dbhost, $db->dbuser, $db->dbpass);
	if(!$conn) {
...
function spi_prepare_import()
{
	...
	$dbResource = mysql_query("SELECT COUNT(".$map['users']->ID.") as num FROM `".$source->dbname."`.".$source->dbuserpfix.$map['users']->table.' '.$map['users ']->where, $dbConn);
	if(!$dbResource) {
		spi_error('Error: Unable to query USERS using the db settings supplied');
		return false;
	}
	$items = @mysql_fetch_array($dbResource, MYSQL_ASSOC);
	if(!$items) {
	...
	$dbResource = mysql_query("SELECT COUNT(".$map['sfforums']->forum_id.") as num FROM `".$source->dbname."`.".$source->dbbasepfix.$map['sfforums']->table.' '.$map['sfforums']->where, $dbConn);
	if(!$dbResource) {

...
(function($) {
	$(function() {
		...
		$('table.wpDataTable').each(function(){
			...
			if(tableDescription.responsive){
				...
				if(!tableDescription.editable){
					dataTableOptions.fnDrawCallback = function(){
				        ...
				        //$('#table_0 tbody').find('tr').each(function(i){
				        	//$(this).append('<a href="link'+i+'" >Button '+i+'</a>')
					    //});
						$.each($('td').not(':empty,.sort'), function(i,v){
			...
			$('#table_0 tbody').on( 'click', '.editLink', function () {
				var row = $(this).closest('tr').get(0);
					aData = wpDataTables[tableDescription.tableId].fnGetData(row),
					vidId = aData[0];
				...
				if(l.pathname.split('/')[1]) {
					...
					base_url = 'http://www.gameveins.com/'
				}
				//window.location.href = base_url + '/gv/edit-video/?pid=' + vidId + '&_wpnonce=' + videoNonce;
				window.open(base_url + '/edit-video/?pid=' + vidId + '&_wpnonce=' + videoNonce,'_blank');
			});
			
			$('#table_0 tbody').on( 'click', '.deleteLink', function () {
				var row = $(this).closest('tr').get(0);
					aData = wpDataTables[tableDescription.tableId].fnGetData(row),
					vidId = aData[0];
				vidId = parseInt(vidId,10);
				...
				//window.location.href = base_url + '/gv/video-manager/?action=del&pid=' + vidId + '&_wpnonce=' + deleteNonce;
				//window.location.href = wpufHref;
				bootbox.confirm({
					title: 'Delete Video',
					...
					callback: function(result) {
						if(result) {
							window.location.href = base_url + '/video-manager/?action=del&pid=' + vidId + '&_wpnonce=' + videoNonce;
						}
					}
				});
			});
